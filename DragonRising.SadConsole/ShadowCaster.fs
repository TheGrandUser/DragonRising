...
let private translateOrigin f x y a b = f (a + x) (b + y)

let private translateOctant f octant = 
   match octant with
   | 0 -> f
   | 1-> fun x y -> f y x
   | 2 -> fun x y -> f -y x
...
let private isInRadius x y radius = square (2 * x - 1) + square (2 * y - 1) <= 4 * square radius

let private computeFoVForColumnPortion current isOpaque setFieldOfView radius (queue: System.Collections.Generic.Queue<ColumnPortion>) = 
...
let private computeFieldOfViewInOctantZero isOpaque setFieldOfView radius = 
